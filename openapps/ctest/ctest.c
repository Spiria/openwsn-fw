...
const uint8_t ctest_ledpath[] = "test";
   ...
   switch(coap_header->Code){
      case COAP_CODE_REQ_GET:
         ...
         switch(currTarget){
            case 0:
               msg->payload[1] = '0';
               openserial_printInfo(COMPONENT_CTEST, ERR_CTEST_TARGET_INFO,
                                     (errorparameter_t)0,
                                     (errorparameter_t)0);
               break;
            case 1:
      ...
      case COAP_CODE_REQ_PUT:
         if(msg->payload[0] == '0'){
            ...
            currTarget = 0;
            openserial_printInfo(COMPONENT_CTEST,ERR_CTEST_TARGET_CHANGED,
                                  (errorparameter_t)0,
                                  (errorparameter_t)0);
           ...
           openserial_printInfo(COMPONENT_CTEST,ERR_CTEST_GOT_PACKET,
                                 (errorparameter_t)2,
                                 (errorparameter_t)0);
            ...
            ctest_vars.tId    = opentimers_start(CTESTPERIOD,
                                                TIMER_PERIODIC,TIME_MS,
                                                ctest_timer_cb);
            openserial_printInfo(COMPONENT_CTEST,ERR_CTEST_ENABLED,
                                  (errorparameter_t)9,
                                  (errorparameter_t)0);
         }
